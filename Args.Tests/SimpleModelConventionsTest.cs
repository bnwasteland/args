...
namespace Args.Tests
{
    ...
    public class SimpleModelConventionsTest
    {
        ...
        public void BasicConventionsTest()
        {
            ...
            member.Parent.Should().Be.EqualTo(m);
            member.SwitchValues.Count().Should().Be.EqualTo(2);
            member.CanHandleSwitch("c").Should().Be.True();
            member.CanHandleSwitch("converterproperty").Should().Be.True();
            member.TypeConverter.Should().Not.Be.Null();
            (member.TypeConverter as ArgsTypeConverter).Converter.GetType().Should().Be.EqualTo<SimpleConverter>();
            member.HelpText.Should().Be.Null();
        }
